...
f=""
class Checkingstrangersandfacialexpression():
    ...
    def run(self):
        ...
        last_smile=facial_expression_label=''
        # 初始化摄像头
        if not input_video:
        ...
        while True:
            ...
            for ((left, top, right, bottom), name) in zip(face_location_list,
                ...
                if name != 'Unknown' and id_card_to_type[name] == 'old_people':
                    ...
                    # print(frame)
                    img = frame.copy() # 保存时使用
                    img=img[top:bottom,left:right]
                    # print(img)
                    # cv2.imwrite(os.path.join(output_smile_path,"temp.jpg"),img)
                    ...
                    for keys, values in b.items():
                        if values == max_value:
                            # print(keys, values)
                            facial_expression_label =keys
                    
                    # facial_expression_label = 'Neural' if neural > smile else 'Smile'
                    

                    if facial_expression_label == last_smile:  # alert
                        if facial_expression_timing == 0:  # just start timing
                            ...
                            if difference < facial_expression_limit_time:
                                print('[INFO] %s, 房间, %s仅%s了 %.1f 秒. 忽略.' % (current_time, id_card_to_name[name],facial_expression_label,difference))
                            else:  # he/she is really smiling
                                event_desc = '%s正在%s' % (id_card_to_name[name],facial_expression_label)
                                event_location = '房间'
                                print('[EVENT] %s, 房间, %s正在%s.' % (current_time, id_card_to_name[name],facial_expression_label))
                                # print(output_smile_path+facial_expression_label)
                                cv2.imwrite(os.path.join(output_smile_path+"%s" %(facial_expression_label),
                                                        'snapshot_%s.jpg' % (time.strftime('%Y%m%d_%H%M%S'))), frame)  # snapshot
